...
![節點圖示](paths-vertex-processor.resources/paths-vertex-processor-icon.png "節點圖示")
...
|  |  |
|:---|:---|
| <b>先前存取的頂點</b> <i>整數</i> | 使用此參數後，你可以透過 <b>Per Vertex Function</b> 參數函式中的 <b>Get</b> 節點，取得路徑上&#x200B;*前一個頂點（prev.pos*）及前一個頂點（*prev[2].pos*）的位置。 |
| <b>下一頂點存取</b> <i>整數</i> | 使用此參數後，你可以利用 <b>Per Vertex Function</b> 參數函式中的 <b>Get</b> 節點，取得路徑上下一個頂點（*next.pos*）及下一個頂點（*next[2].pos*）的位置。 |
| <b>影像輸入計數</b> <i>整數</i> | 用於連接應在每個頂點函</b>數參數函數中<b>取樣的影像的可見 <b>Input #</b> 輸入連接器數量。<br>當你完成所有想要的取樣後，可以透過將這個參數的值降回 0 來隱藏未使用的腳位。 |
...
![節點範例 2](paths-vertex-processor.resources/PathsVertexProcessor-Demo2.gif "節點範例 2")
