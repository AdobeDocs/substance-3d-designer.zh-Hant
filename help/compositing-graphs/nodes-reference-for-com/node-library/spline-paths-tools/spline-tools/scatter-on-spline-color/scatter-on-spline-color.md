...
*- 加法*：將顏色相加;
* *Alpha 混合*：利用圖案的 alpha 通道進行簡單的透明混合。 最後畫出的圖案在前面。

<b>彩色模式</b> *整數*&#x200B;混合方法：選擇每個圖案的顏色：\
*- 底色*：底色會套用在所有圖案上;
* *位置*：圖案在貼圖空間中的位置用來驅動其顏色，使 X 與 Y 座標分別映射到紅色與綠色通道。

<b>形狀底色</b> *Float4*&#x200B;圖案的底色。

<b>色彩輸入倍增器</b> *浮點*&#x200B;控制色彩映射輸入的強度。 此映射作為圖案當前顏色的乘數。\
此映射的效果與「顏色」組中其他參數結合。\
注意：輸出顏色是所有色彩乘數的加權結果。

<b>色彩圖輸入取樣模式</b> *整數*&#x200B;將色彩映射中值映射到樣條的方法：\
*- 貼圖空間*：這些值會套用到樣條（spline）上，若使用貼圖的 UV 座標放置於貼圖中，該點樣條會放在的位置。 這實際上將值套用到「已就位」的樣條曲線上;\
*- 水平沿樣條線*：數值直接套用到編碼的樣條座標（參見樣條座標輸入），每列從上到下分別套用到不同的樣條曲線;\
*- 霍爾。 沿樣條曲線（蘭德。 偏移量 X）：*&#x200B;這些值直接套用到編碼後樣條線的座標上（參見樣條座標輸入），每個樣條線（即樣條座標中的每一列）在色彩圖中隨機進行水平偏移;\
*- 霍爾。 沿樣條曲線（蘭德。 偏移量 Y）：*&#x200B;這些值直接套用到編碼後樣條線的座標（參見樣條座標輸入），並在色彩映射中對每個樣條線（即樣條座標中的每一列）隨機垂直偏移。

<b>隨機色彩</b> *Float4*&#x200B;會對圖案在 HSV 空間中的顏色以及其 alpha 值施加隨機偏移，最多可達指定值。\
*注意：* 輸出顏色是所有色彩乘數的加權結果。

<b>隨機色彩中心</b> *浮動*&#x200B;對隨機色彩中隨機偏移的範圍施加偏移\
值為 -1 表示所有隨機值都較高，1 則表示所有隨機值較低。
...
<b>樣條厚度倍增器</b> *浮點*&#x200B;每個圖案的α值與樣條厚度在該位置相乘的強度。\
注意：輸出顏色是所有色彩乘數的加權結果。

<b>形狀尺度倍增器</b> *浮動*&#x200B;指每個圖案的 alpha 相乘於其尺度的強度。\
注意：輸出顏色是所有色彩乘數的加權結果。

<b>形狀指數乘數</b> *浮點*&#x200B;指每個圖案的α相乘於其正規化指數的強度。\
注意：輸出顏色是所有色彩乘數的加權結果。

<b>花鍵高度倍數</b> *浮點*&#x200B;每個圖案的α值與樣條線在該位置高度相乘的強度。\
注意：輸出顏色是所有色彩乘數的加權結果。

<b>隨機亮度</b> *浮點*&#x200B;會套用隨機乘數，最高可達指定值，以降低模式的 alpha。\
注意：輸出顏色是所有色彩乘數的加權結果。
