---
helpx_url: "https://helpx.adobe.com/tw/substance-3d-designer/substance-compositing-graphs/nodes-reference-for-substance-compositing-graphs/node-library/filters/effects/flood-fill-to-position.html"
breadcrumb-title: ''
description: 使用 Flood Fill to Position 節點，將區域填充基於位置的值，以建立程序模式。
helpx_creative_field: ""
helpx_description: Designer > Substance compositing graphs > Nodes reference for Substance compositing graphs > Node library > Filters > Effects > Flood Fill to Position
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: 淹水填補至位置
user-guide-description: ''
user-guide-title: ''
source-git-commit: 2ea5b90ca7a4cbcd0b0049b156e8ded334aa0ceb
workflow-type: tm+mt
source-wordcount: '78'
ht-degree: 2%

---


# 淹水填補至位置

<table>
<tr style="border: 0;">
<td width="33.33%" style="border: 0;" valign="top">

![](flood-fill-to-position.resources/floodfill-to-position.png){width="128px"}

<b>收錄於：</b> 濾鏡>效應

</td>
<td width="100.00%" style="border: 0;" valign="top">

## 說明

從 [洪水填土](../../../../../../compositing-graphs/nodes-reference-for-com/node-library/filters/effects/flood-fill/flood-fill.md) 基地生成每格位置圖。

每個圖塊的顏色代表其 X 與 Y 座標中心，儲存在紅色和綠色通道中。 此地圖旨在作為後續計算的基礎，而非現成可用的地圖。

</td>
</tr>
</table>

## 範例

<table style="margin-top: 32px; margin-bottom: 32px">
    <tr style="border: 0">
        <td style="border: 0; background: transparent">
            <img src="flood-fill-to-position.resources/floodfill-to-position-1.png" />
        </td>
        <td style="border: 0; background: transparent">
            <img src="flood-fill-to-position.resources/floodposition-ex2.png" />
        </td>
        <td style="border: 0; background: transparent">
            <img src="flood-fill-to-position.resources/floodposition-ex1.png" />
        </td>
    </tr>
</table>
