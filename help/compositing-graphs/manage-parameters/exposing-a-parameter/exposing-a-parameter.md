...
![參數曝光簡化](exposing-a-parameter.resources/parameters-5.png "參數曝光簡化")
...
![單曝光方法攻略](exposing-a-parameter.resources/single-expose2.gif "單張曝光法攻略"){width="512px"}
...
1. 請按 <b>確定</b> 鍵以確認
1. 參數名稱變 *為藍色*，且 ![](exposing-a-parameter.resources/image2020-9-17-15-35-46.png)\
...
![批次暴露方法攻略](exposing-a-parameter.resources/batch-expose-2.gif "批次暴露方法攻略"){width="512px"}

### 批次曝光法

當暴露一個參數時，此方法會比前一種稍慢。 當暴露多個參數時，速度會快得多。

1. 不要只找單一參數，而是在「特定參數</b>」標籤右上角<b>的![](exposing-a-parameter.resources/image2020-9-17-15-39-7.png)<b>多重曝光</b>按鈕
...
1. 點擊 <b>確定</b> 以確認並顯示所有選取的參數。 參數名稱現在會顯示&#x200B;*藍色*，以確認參數已被揭露，並且有![](exposing-a-parameter.resources/image2020-9-17-15-35-46.png)<b>編輯功能</b>按鈕。
...
1. 點擊已曝光參數旁的下拉選項按鈕![](exposing-a-parameter.resources/image2020-9-17-15-35-59.png)
1. 選擇 ![](exposing-a-parameter.resources/image2020-9-17-15-37-7.png)<b> 編輯暴露的圖形輸入</b>。 這會直接帶你到圖屬性中相關的項目
1. 在圖的空白區域雙擊進入圖屬性，然後在輸入參數清單 <b>中找到該參數</b>
1. 在檔案總管</b>中單擊你的圖表<b>，然後在輸入參數列表中<b>找到該參數</b>

![輸入參數](exposing-a-parameter.resources/input-parameters-2.png "輸入參數"){width="512px"}
...
![整數參數](exposing-a-parameter.resources/dropdown.gif "下拉清單編輯器整數參數下拉清單編輯器"){width="512px"}
...
![重新排序輸入參數](exposing-a-parameter.resources/reorder-2.gif "重新排序輸入參數"){width="512px"}

#### 重新訂購

參數的重新排序可以輕鬆地透過拖放輸入參數名稱左側的深色條紋握柄來完成。 要注意，分組參數會影響順序。

![預視輸入參數](exposing-a-parameter.resources/parameter-preview-2.gif "預視輸入參數"){width="512px"}
...
可透過複製按鈕 ![](exposing-a-parameter.resources/image2019-9-19-11-3-49.png)複製單一參數。 多個參數可透過參數選單 ![](exposing-a-parameter.resources/image2020-9-17-15-39-7.png)複製。 選擇複製輸入以複製所有輸入。

在參數選單![](exposing-a-parameter.resources/image2020-9-17-15-39-7.png)中選擇「貼入輸入」![](exposing-a-parameter.resources/image2020-9-17-16-43-15.png)來貼上一個或多個參數。
...
![節點參數](exposing-a-parameter.resources/parameter-error.gif "錯誤節點參數錯誤"){width="512px"}

### 追蹤節點上破損的參數

你可以透過節點搜尋工具追蹤哪個節點使用的參數，該工具 ![](exposing-a-parameter.resources/image2019-9-19-14-15-53.png)位於圖圖檢視的頂欄。 點擊它，你可以用特定參數找到節點。

如果節點真的有問題，會在左上角顯示警告徽章 ![](exposing-a-parameter.resources/image2019-9-19-14-23-54.png) 。 將滑鼠移到徽章上會顯示一個提示，裡面有更多資訊。

要重設並移除問題，針對你想修正或重設的參數，請點擊編輯功能按鈕旁的下拉選![](exposing-a-parameter.resources/image2020-9-17-15-35-59.png)單，選擇![](exposing-a-parameter.resources/image2020-9-17-16-56-18.png)<b>重設。</b>這會將參數回復到先前未曝光的狀態，藍色名稱會再次變為灰色以反映此情況。

![清理未使用的輸入參數](exposing-a-parameter.resources/clean-inputs-2.gif "清理未使用的輸入參數"){width="512px"}

### 清理未使用的輸入參數

如果你已經失去輸入參數的蹤跡，不知道哪些參數被使用，可以用一個小工具清理。 點擊輸入參數選單按鈕 ![](exposing-a-parameter.resources/image2020-9-17-15-39-7.png) ，選擇 <b>乾淨輸入。</b>

會跳出一個新對話框，列出所有未使用的參數。 勾選或取消你想移除或保留的參數，然後點選確定。 如果沒有對話框，表示目前沒有未使用的參數需要清理。

![移除參數](exposing-a-parameter.resources/delete-param-2.gif "移除參數"){width="512px"}

### 移除參數

要實際移除正在使用的參數，需要兩個不同的步驟。

1. 在有暴露參數的節點上，點擊「函式暴露」按鈕右側藍色的下拉箭頭： ![](exposing-a-parameter.resources/image2019-9-19-14-55-55.png)。 然後選擇「重置為預設值」。 這樣就不用再用這個節點的參數。 對使用相同參數的其他節點重複此法。 「Reset to Default value」也會將參數小工具的範圍重置為軟 *範圍*。
