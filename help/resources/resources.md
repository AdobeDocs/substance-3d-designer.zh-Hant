---
helpx_url: "https://helpx.adobe.com/substance-3d-designer/resources.html"
breadcrumb-title: ''
description: 學習如何在 Substance 3D Designer 中匯入、建立及管理資源，包括點陣圖、字型與 3D 場景。
helpx_creative_field: ""
helpx_description: Designer > Resources
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: 資源
user-guide-description: ''
user-guide-title: ''
source-git-commit: fde9d7a455c1c7b366323c119f4c1f9a2c114952
workflow-type: tm+mt
source-wordcount: '155'
ht-degree: 0%

---


# 資源

資源是 Substance 3D Designer 套件的內容。 它們用於建構、產生及預覽材質與材質資料。 資源可分為兩大類：圖資源與非圖資源。 不同類型建立或匯入資源的方法不同，你可以閱讀專門的 [連結、匯入與新資源](../resources/importing-linking-and-new/importing-linking-and-new-resources.md) 頁面以更了解。

<table>
<tr style="border: 0;">
<td width="16.67%" style="border: 0;" valign="top">

![](resources.resources/graph-5.png)

</td>
<td width="100.00%" style="border: 0;" valign="top">

## 圖資源

圖資源是 Designer 內建的主要資源。 每個版本在文件中都有專屬區域。 主要有三種類型：

* [物質圖](../compositing-graphs/substance-compositing-graphs.md)
* [函數圖](../function-graphs/function-graphs.md)

</td>
</tr>
</table>

<table>
<tr style="border: 0;">
<td width="16.67%" style="border: 0;" valign="top">

![](resources.resources/folder-4.png)

</td>
<td width="100.00%" style="border: 0;" valign="top">

### 非圖資源

非圖資源通常是輔助資源，作為圖中的額外資料，或用來幫助視覺化圖表結果。 存在以下類型，每種類型在文件的這個章節都有子頁面：

* [位圖](bitmap-resource/bitmap-resource.md)
* [向量圖形（SVG）](vector-graphics-svg-res/vector-graphics-svg-resource.md)
* [3D 場景資源](3d-scene-resource/3d-scene-resource.md)
* [洗禮盆](font-resource/font-resource.md)

</td>
</tr>
</table>
